...
app.post('/api/verify-attendance', (req, res) => {
    ...
    dbManager.db.students.findOne({ matricNo: targetStudentId }, (studentErr, studentRecord) => {
        ...
        // Dynamically extract the registered English name from the database row
        const studentName = studentRecord.name; 

        // Create a Case-Insensitive Regular Expression for NeDB session matching
        const courseRegex = new RegExp(`^${targetCourse}$`, 'i');

        // 2. Look up the active session using the flexible regex pattern
        dbManager.db.sessions.findOne({ courseCode: courseRegex, isActive: true }, (err, activeSession) => {
            ...
            // 3. Compute Telemetry Verification Metrics against dynamic coordinates
            const distance = getDistanceInMeters(latitude, longitude, activeSession.latitude, activeSession.longitude);
            console.log(`\n[Incoming Request] Student Match Found: ${studentName} (${targetStudentId})`);
            console.log(`Calculated physical distance from live target: ${distance.toFixed(1)} meters`);
            ...
            if (distance > 500) {
                ...
                let noiseScore = 0.0;
                const NOISE_TOLERANCE_RANGE = 15.0; // Allowed variance window (+/- 15 units)

                if (noiseLevel === 0) {
                    // Hardware bypass: If microphone permission was denied or failed to initialize
                    noiseScore = 1.0; 
                } else {
                    const noiseDifference = Math.abs(noiseLevel - activeSession.baseNoise);

                    if (noiseDifference <= NOISE_TOLERANCE_RANGE) {
                        // Full marks if within the acceptable classroom profile range
                        noiseScore = 1.0; 
                    } else {
                        // Scale down dynamically if they are outside the range, up to a maximum variance of 40
                        noiseScore = Math.max(0, 1 - ((noiseDifference - NOISE_TOLERANCE_RANGE) / 40.0));
                    }
                }

                // Compute dynamic temperature metrics matching initialized session base parameters
                const currentBaseTemp = activeSession.baseTemp || 26.2;
                const tempDiff = Math.abs(temperature - currentBaseTemp);
                const tempScore = Math.max(0, 1 - (tempDiff / 3.0));

                // Calculate total weighted score matrix
                score = (noiseScore * 0.5) + (tempScore * 0.5);
                verified = score >= 0.5;
            }

            const message = verified 
                ? `Attendance Successfully Verified! Logged into system core database.`
                : "Attendance Denied. Environmental metrics or Geofencing parameters do not match the classroom profile.";

            // 4. Persist log row history using the dynamic matching name
...
app.post('/api/create-session', (req, res) => {
    ...
    dbManager.createActiveSession(courseCode, latitude, longitude, baseNoise, durationMinutes, (err, newSession) => {
        ...
        console.log(`\n[Database Configuration] New tracking perimeter initialized for ${courseCode} at (${latitude}, ${longitude}).`);
